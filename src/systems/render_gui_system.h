//
// Created by Hector Mejia on 12/20/23.
//
    ...
    void Update(const std::unique_ptr<Registry>& registry, const SDL_Rect camera) {
        ...
        if (ImGui::Begin("Spawn enemies")) {
            ...
            if (ImGui::Button("Spawn new enemy")) {
                ...
                const double projVelX = cos(projAngle) * projSpeed; // convert from angle-speed to x-value
                const double projVelY = sin(projAngle) * projSpeed; // convert from angle-speed to y-value
